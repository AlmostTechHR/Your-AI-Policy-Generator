...
function downloadPolicy() {
    ...
    const doc = new jsPDF();
     ...
     // Add your custom font (place these lines immediately after creating the doc)
    // Replace "MyUnicodeFont.ttf" and "BASE64_ENCODED_FONT_DATA" with your font's filename and its Base64 data.
    doc.addFileToVFS("MyUnicodeFont.ttf", "BASE64_ENCODED_FONT_DATA");
    doc.addFont("MyUnicodeFont.ttf", "MyUnicodeFont", "normal");
    doc.setFont("MyUnicodeFont", "normal");
    ...
    // Helper: Add wrapped text (splits the provided text and writes each line)
    ...
    doc.setFontSize(16);
    addWrappedText(`AI Readiness Report for ${orgName}`);
    y += 5; // extra spacing

    doc.setFontSize(12);
    addWrappedText(`Hi team at ${orgName},`);
    addWrappedText(`Here's a quick snapshot of your current readiness to adopt AI in an ethical, inclusive, and responsible way.`);
    y += lineHeight; // Add extra space

    doc.setFont(undefined, 'bold');
    addWrappedText(`✨ Your Readiness Score: ${score} / 100`);
    doc.setFont(undefined, 'normal');
    y += lineHeight;

    if (score >= 75) {
        addWrappedText(`✅ You're doing great!`);
        addWrappedText(`You're building on strong AI practices.`);
    } else if (score >= 50) {
        addWrappedText(`🟡 You’re progressing well.`);
        addWrappedText(`Keep growing your practices around training and compliance.`);
    } else {
        addWrappedText(`🔴 A great time to start!`);
    ...
    doc.setFont(undefined, 'bold');
    addWrappedText("You Might Consider:");
    doc.setFont(undefined, 'normal');
    ...
    doc.setFontSize(16);
    addWrappedText(`Your Draft Responsible AI Policy`);
    y += 5;
